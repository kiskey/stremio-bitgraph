...
// TMDB_BASE_URL and TMDB_API_KEY are now directly accessed from config
// No need to redeclare constants here if config is always available.
...
async function getTvShowDetails(tmdbId) {
  ...
  try {
    const response = await retryWithExponentialBackoff(
      async () => axios.get(url, {
        params: {
          api_key: config.tmdb.apiKey, // Uses server's TMDB API key from config
